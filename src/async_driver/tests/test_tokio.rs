...
fn try_from_shared_handle_succeeds() {
    ...
    assert!(TokioDriver::try_from(shared).is_ok());
}
...
fn try_from_reactor_thread_always_returns_logic_error() {
    assert!(matches!(
        TokioDriver::try_from(make_reactor_thread()),
...
fn try_from_reactor_thread_error_message_mentions_reactor_thread() {
    match TokioDriver::try_from(make_reactor_thread()) {
        ...
        Ok(_) => panic!("Expected FluxError::Logic, got an actual TokioDriver object."),
...
// =========================================================================
// AsyncDriver::spawn / stop lifecycle (require Tokio runtime)
// =========================================================================

#[tokio::test]
async fn spawn_succeeds() {
    let shared = make_shared_handle();
    let mut driver = TokioDriver::try_from(shared).unwrap();
    assert!(driver.spawn().is_ok());
    driver.stop().unwrap();
}

#[tokio::test]
async fn spawn_takes_handle_leaving_none() {
...
#[tokio::test]
async fn spawn_sets_task_handle() {
    // After spawn(), the Tokio JoinHandle should be stored in task_handle.
    let shared = make_shared_handle();
    let mut driver = TokioDriver::try_from(shared).unwrap();
    driver.spawn().unwrap();
    assert!(
...
#[tokio::test]
async fn stop_after_spawn_succeeds() {
    let shared = make_shared_handle();
    let mut driver = TokioDriver::try_from(shared).unwrap();
    driver.spawn().unwrap();
...
#[tokio::test]
async fn stop_clears_task_handle() {
    // After stop(), task_handle should be None since Option::take was called.
    let shared = make_shared_handle();
    let mut driver = TokioDriver::try_from(shared).unwrap();
    driver.spawn().unwrap();
    driver.stop().unwrap();
...
#[tokio::test]
async fn stop_without_spawn_succeeds() {
...
#[tokio::test]
async fn stop_without_spawn_is_idempotent() {
    let shared = make_shared_handle();
    let mut driver = TokioDriver::try_from(shared).unwrap();
    driver.stop().unwrap();
    assert!(driver.stop().is_ok());
...
#[tokio::test]
async fn second_spawn_fails_with_logic_error() {
...
#[tokio::test]
async fn spawn_async_driver_succeeds() {
    let shared = make_shared_handle();
    let result = TokioDriver::spawn_async_driver(shared);
    assert!(result.is_ok());
    result.unwrap().stop().unwrap();
}

#[tokio::test]
async fn spawn_async_driver_handle_is_none() {
    // spawn_async_driver calls spawn() which consumes the handle field.
    let shared = make_shared_handle();
    let driver = TokioDriver::spawn_async_driver(shared).unwrap();
    assert!(
        driver.handle.is_none(),
...
#[tokio::test]
async fn spawn_async_driver_task_handle_is_some() {
    // spawn_async_driver calls spawn() which populates task_handle.
    let shared = make_shared_handle();
    let driver = TokioDriver::spawn_async_driver(shared).unwrap();
    assert!(
        driver.task_handle.is_some(),
...
#[tokio::test]
async fn spawn_async_driver_produces_running_driver() {
    // A successfully spawned driver should be stoppable without error.
    let shared = make_shared_handle();
    let mut driver = TokioDriver::spawn_async_driver(shared).unwrap();
    assert!(driver.stop().is_ok());
}
...
#[tokio::test]
async fn drop_without_stop_does_not_panic() {
    // Drop calls stop() which aborts the task. Must not panic.
    {
        let shared = make_shared_handle();
        let mut driver = TokioDriver::try_from(shared).unwrap();
        driver.spawn().unwrap();
    } // driver dropped here — stop() fires in Drop, aborting the task
}

#[tokio::test]
async fn drop_without_spawn_does_not_panic() {
    {
        let shared = make_shared_handle();
        let _driver = TokioDriver::try_from(shared).unwrap();
    } // driver dropped here without spawn() having been called
}

#[tokio::test]
async fn drop_after_explicit_stop_does_not_panic() {
    ...
    // task_handle is already None.
    {
        ...
        driver.spawn().unwrap();
        driver.stop().unwrap();
    } // driver dropped here — Drop calls stop() again on None task_handle
}
